...
class GameState:
    ...
    def get_action_space(self, sid: str) -> List[str]:
        """Returns physically possible moves, completely hard-filtering lethal hazards."""
        ...
        # Battlesnake dynamic hazard damage calculation
            ...
            if 0 <= nx < self.board_width and 0 <= ny < self.board_height:
                ...
                if target not in obs:
                    if target in self.hazards and snake.health <= lethal_dmg:
                        lethal_moves.append(m) # Guaranteed death, prune if possible
                    else:
                        moves.append(m)
                        
        return moves if moves else (lethal_moves if lethal_moves else ["up"])

    def get_guided_move(self, sid: str) -> str:
        """Rollout Guide: Multi-Tier Hazard and Danger Zone Paranoia."""
        ...
        # Tier 1: Avoid Hazards AND Danger Zones
        best_moves = [m for m in safe_moves if (head[0]+MOVES[m][0], head[1]+MOVES[m][1]) not in self.hazards and (head[0]+MOVES[m][0], head[1]+MOVES[m][1]) not in danger_zone]

        if not best_moves:
            # Tier 2: Forced Trade-offs
            if snake.health > 30:
                # If healthy, take the guaranteed 15 hazard damage rather than a 50/50 instant death
                best_moves = [m for m in safe_moves if (head[0]+MOVES[m][0], head[1]+MOVES[m][1]) not in danger_zone]
            else:
                # If dying anyway, risk the head-to-head collision over the guaranteed hazard death
                best_moves = [m for m in safe_moves if (head[0]+MOVES[m][0], head[1]+MOVES[m][1]) not in self.hazards]

        choices = best_moves if best_moves else safe_moves

        # Path to food if health is dropping
...
# ==========================================
# Hazard-Paranoid MCTS Architecture
# ==========================================
class MCTSNode:
    ...
    def expand(self):
        ...
        elif me.head in next_state.hazards:
            # HAZARD PARANOIA: Massive penalty to Progressive Bias. 
            # UCB equation will shun this branch unless all others are proven deaths.
            child_node.pb_score -= 2.0 
    ...
    def simulate(self, depth_limit: int = 15) -> float:
        ...
        if not me or not me.is_alive: 
            return 0.0 # Death is an absolute zero, no partial credit for surviving a few turns
            
        survival = 1.0
        health_bonus = me.health / 200.0
        length_bonus = me.length / 50.0
        
        # Severe penalty if rollout survived but ended in a hazard or near death
        hazard_penalty = 0.0
        if me.head in current_state.hazards:
            hazard_penalty -= 0.5
        if me.health < 20:
            hazard_penalty -= 0.5
            
        return survival + health_bonus + length_bonus + hazard_penalty
...
class MCTSAgent:
    ...
    def search(self, root_state: GameState) -> str:
        ...
        print(f"MCTS-Hazard-Proof Iterations: {iterations} | Time: {time.time() - start_time:.3f}s")
...
# ==========================================
def info() -> Dict:
    return {"apiversion": "1", "author": "MGAIA_MCTS_2", "color": "#2427e3", "head": "default", "tail": "default"}
